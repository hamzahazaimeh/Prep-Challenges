...
const cvsFiltration = (arr) => {
    ...
    const result = arr
    .filter((student) => {
      return student.tech === "JS" && student.yearsOfExperience >= 4;
    })
    .map((filteredStudent) => {
      return {
        fullName: `${filteredStudent.firstName} ${filteredStudent.LastName}`,
        tech: filteredStudent.tech,
      };
    });
  return result;
};
...
const vowelsFiltration = (arr) => {
    const myRegex = /[a,e,i,o,u]/;
    const result = arr.filter((word) => {
      return !myRegex.test(word);
    });
    return result;
